package unit2;//《数据结构与算法（Java版）（第5版）》，作者：叶核亚，2019年8月8日
//§3.2.1   常量字符串
//3．MyString常量字符串类
//【思考题3-1】MyString类，trim()
//【思考题3-1，习题解答】串比大小，忽略大小写
//【实验题3-1】MyString类增加成员方法
//§3.3.1 Brute-Force算法
//【思考题3-4】替换子串
...
public final class MyString implements Comparable<MyString>, java.io.Serializable {
    ...
    public MyString(String s)          //由字符串常量构造串
    {
...
//        this.value = java.util.Arrays.copyOf(value,value.length);       //复制数组，包括申请数组空间
    ...
    public char charAt(int i) {
        if (i >= 0 && i < this.value.length) return this.value[i];
    ...
    public MyString concat(MyString s) {
        if (s == null || s.equals("")) s = new MyString(this.value);        //深拷贝this
        char[] buffer = new char[this.value.length + s.length()];
        int i;
        for (i = 0; i < this.value.length; i++)     //复制当前串
            buffer[i] = this.value[i];
        //复制指定串s
        System.arraycopy(s.value, 0, buffer, i, s.value.length);
    ...
    public int compareTo(MyString s) {
        for (int i = 0; i < this.value.length && i < s.value.length; i++)
            if (this.value[i] != s.value[i]) return this.value[i] - s.value[i];       //返回两串第一个不同字符的差值
        return this.value.length - s.value.length;       //前缀子串，返回两串长度的差值
    }
    //【例3.1】  使用String串的求子串和连接串操作，实现串的插入、删除功能。

    //【思考题3-1】
/*    public Unit2.MyString trim()                       //返回删除所有空格后的字符串。使用字符数组
    {
        Unit2.MyString result = new Unit2.MyString("");
    ...
    public MyString trim()                       //返回删除所有空格后的字符串。使用字符数组
    {
        ...
        int n = 0;
        //复制this串一次
        for (char c : this.value) if (c != ' ') buffer[n++] = c;
    ...
    public int compareToIgnoreCase(MyString str) //比较两个串大小，忽略字母大小写
    {
        for (int i = 0; i < this.value.length && i < str.value.length; i++) {
            ...
            if (c1 >= 'a' && c1 <= 'z') c1 -= 'a' - 'A';
            if (c2 >= 'a' && c2 <= 'z') c2 -= 'a' - 'A';
            if (c1 != c2) return this.value[i] - str.value[i];
        }
...
/*        int j=suffix.value.length-1;
    ...
    public boolean equalsIgnoreCase(MyString str)//比较this串与str串是否相等，忽略字母大小写
    {
        ...
        if (this.value.length != str.value.length) return false;
        for (int i = 0; i < this.value.length; i++) {
...
/*    public Unit2.MyString replaceFirst(Unit2.MyString pattern, Unit2.MyString str)
    {
...
习题3 未完成
public Unit2.MyString replace(char old, char newc)      	//将串中所有old字符替换为newc字符
...
*/
//@author：Yeheya。2014年6月25日，2015-9-23，2019年9月28日，2020年2月13日