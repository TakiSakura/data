...
public final class MyStringBuffer implements java.io.Serializable {
    ...
    {
        if (i >= 0 && i < this.n) return this.value[i];
        throw new StringIndexOutOfBoundsException(i);
    }

    public void setCharAt(int i, char ch)        //设置第i个字符为ch，0≤i<length()
    {
        if (i >= 0 && i < this.n) this.value[i] = ch;
    ...
    //2.  插入串
    //在第i个字符处插入s串，0≤i<length()。若i序号越界，则抛出字符串序号越界异常；若s==null，则插入""
    public synchronized MyStringBuffer insert(int i, String s) {
        if (this.n == 0 && i == 0 || this.n > 0 && i >= 0 && i <= this.n) {
            ...
            if (this.n - i >= 0) System.arraycopy(temp, i, this.value, i + s.length(), this.n - i);
    ...
    public synchronized MyStringBuffer delete(int begin, int end) {
        if (begin >= 0 && begin < this.n && end >= 0 && begin <= end) {
            ...
            //从end开始至串尾的子串向前移动
            System.arraycopy(this.value, end + 0, this.value, begin + 0, this.n - end);
            this.n -= end - begin;
            return this;
        } else
...
//@author：Yeheya。2014年10月3日，2019年8月8日