...
public class SeqList<T> implements Iterable<T>//,List<T>   //10.2.2，实现可迭代接口
//public class Unit1.SeqList<T>  extends MyAbstractList<T> //顺序表类，继承抽象列表类。提供迭代器对象
{
...
//      super();                                 // 默认调用，执行Object()
    ...
    public SeqList(T[] values)                   // 构造顺序表，由values数组提供元素，忽略其中空对象。O(n)
    {
        ...
        // 复制非空的数组元素
        for (T value : values) if (value != null) this.element[this.n++] = value;// 对象引用赋值
    }
    ...
    {
        if (i >= 0 && i < this.n) return (T) this.element[i];           // 返回数组元素引用的对象，传递对象引用
//            return this.element[i];              //编译错，Object对象不能返回T对象
        return null;
        //不能抛出异常。因为null，可以作为循环不执行的条件。
    }

    // 若0≤i<n且x≠null，设置第i个元素为x；O(1)  ////不要返回值。
    public void set(int i, T x) {
        if (x == null) throw new NullPointerException("x==null");     //抛出空对象异常
        if (i >= 0 && i < this.n) element[i] = x;                           //对象引用赋值
...
//        String str=this.getClass().getName()+"(";//返回类名
        ...
        if (this.n > 0) str.append(this.element[0].toString());   // 执行T类的toString()方法，运行时多态
...
//        String str=this.getClass().getName()+"(";       // 返回类名
        StringBuilder str = new StringBuilder("(");
        if (this.n > 0) str.append(this.element[this.n - 1].toString());      // 执行T类的toString()方法，运行时多态
    ...
    public int insert(int i, T x) {
        if (x == null) return -1;                           // 没有插入。//返回一种执行结果，不是错误，不抛出空对象异常
        if (i < 0) i = 0;                                 // 插入位置i容错，插入在最前（头插入）
        if (i > this.n) i = this.n;                            // 插入在最后（尾插入）
...
//X4 -> O8 -> P3 -> I5
    //6.  顺序表的浅拷贝与深拷贝
/*    public Unit1.SeqList(Unit1.SeqList<T> list)              // 浅拷贝构造方法
    {
...
//因为，子类SortedSeqList(Unit1.SeqList<? extends T> list)中super(list)，insert(x)运行时多态，遍历到尾再插入，效率O(n*n)
    ...
    public boolean equals(Object obj)            // 比较this与obj引用的顺序表是否相等。覆盖。O(n)
    {
        ...
        if (this == obj)                            // 若this和obj引用同一个顺序表实例，则相等
            return true;
        if (obj instanceof SeqList<?>)            // 若obj引用顺序表实例。Unit1.SeqList<?>是所有SeqList<T>的父类
        {
...
//如果  public void addAll(Unit1.SeqList<T> list)          //则不接受T的子类
//如果  public void addAll(Unit1.SeqList<?> list)          //则调用this.insert(x)，编译错
//如果  public void insert(Unit1.SeqList<T> list)          //重载，调用时，当参数为null时，编译错，无法识别重载方法中的哪一个

    //返回在this之后连接list的顺序表，即返回this+list的并集功能，不改变this和list
    public SeqList<T> union(SeqList<? extends T> list) {
        SeqList<T> result = new SeqList<>(this);   //深拷贝this，无法创建子类实例
        result.addAll(list);                        //顺序表合并连接，尾插入
        return result;                              //只能返回SeqList<T>对象，子类要覆盖
    }
    //不行        return new Unit1.SeqList<T>(this).addAll(list);
    //9.5.1 子类mergeAll合并、归并，不用，不声明

    //【实验题2-1】第5版没写
    //返回从begin～end元素组成的子表。意为返回T的某个子类。
//  Unit1.SeqList<? extends T> subList(int begin, int end)   //不需要该语法
    ...
    private class SeqIterator implements java.util.Iterator<T>// 私有内部类，实现迭代器接口
    {
        ...
        {
            return this.succ < SeqList.this.n;     // Unit1.SeqList.this.n是外部类当前实例的成员变量
        }
    ...
    private class SeqListIterator extends SeqIterator implements java.util.ListIterator<T> {
        ...
        {
            if (this.index >= 0 && this.index < SeqList.this.n) SeqList.this.set(this.index, x); //调用外部类当前实例的成员方法
            else throw new IllegalStateException(); //抛出无效状态异常
        }
