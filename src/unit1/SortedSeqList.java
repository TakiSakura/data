package unit1;//《数据结构与算法（Java版）（第5版）》，作者：叶核亚，2019年7月23日
//§2.4   排序线性表的存储和实现
//§2.4.1  排序顺序表
//第5版教材没写，【例8.1】用升序
...
//        this(values.length);        
        //直接插入排序，每趟插入1个元素
        for (T value : values) this.insert(value);              //调用子类覆盖的insert(T)方法，按值插入元素，O(n)
    }//§9.5.1   顺序表的排序算法
    ...
    {
        super(slist);                  //调用SeqList(Unit1.SeqList<T> list)，参数list=slist赋值相容，list引用子类实例
    }
...
//    }    
    ...
    ////以下【研究】，第5版没写
//    public void addAll(Unit1.SeqList<? extends T> list)      //继承

    //    public Unit1.SeqList<T> union(Unit1.SeqList<? extends T> list)   //继承，不行
    //覆盖，返回值类型不同但赋值相容，包含参数SortedSeqList<? extends T> 
    public SortedSeqList<T> union(SeqList<? extends T> list) {
        SortedSeqList<T> result = new SortedSeqList<>(this);   //创建子类实例，深拷贝父类实例。只此一句不同
...
/*    //注意，以下3方法，程序好。深拷贝仅复制数组，没有复制对象，操作后存在共用对象问题
    public Unit1.SortedSeqList(T[] values)             //构造排序顺序表，由values数组提供元素
    {
...
//                                                 //编译错，因为不能将Object[]转换成T[]        
    ...
    //由顺序表list构造排序顺序表。
//    public Unit1.SortedSeqList(Unit1.SeqList<? extends T> list)    //Unit1.SeqList<T>中的T，是SortedSeqList<T>类声明的T，可比较大小
    public Unit1.SortedSeqList(Unit1.SeqList<T> list)        //Unit1.SeqList<T>中的T，是SortedSeqList<T>类声明的T，可比较大小
    {
...
*/
    ...
    //第5版教材方法体省略
//    public void merge(Unit1.SortedSeqList<? extends T> list)
...
//    public Unit1.SortedSeqList<T> mergeWith(Unit1.SortedSeqList<? extends T> list)
    public SortedSeqList<T> mergeWith(SortedSeqList<T> list) {
        SortedSeqList<T> templist = new SortedSeqList<>((this.n + list.n) * 2);
        int i = 0, j = 0, k = 0;
        while (i < this.n && j < list.n)
