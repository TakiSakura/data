...
//public class Unit1.SinglyList<T> extends MyAbstractList<T>//单链表类，继承抽象列表类//第10章，10.2 实现迭代器
{
    public final Node<T> head;                         // 头指针，指向单链表的头结点

    //注意，head不能是其他权限，因为7.2.2节，图的邻接表中，删除顶点要遍历。
    //（1）构造方法
    ...
    public T get(int i)                          //返回第i个元素，0≤i<单链表长度。O(n)
    {
    ...
    public void set(int i, T x) {
        if (x == null) throw new NullPointerException("x==null");     //抛出空对象异常
            ...
            for (int j = 0; p != null && j < i; j++)  //遍历寻找第i个结点（p指向）
                p = p.next;
            if (i >= 0 && p != null) p.data = x;                       //p指向第i个结点
            else {
...
//        String str=this.getClass().getName()+"(";//返回类名
        for (Node<T> p = this.head.next; p != null; p = p.next)
            str.append(p.data.toString()).append(p.next != null ? "," : "");//不是最后一个结点时，加“,”分隔符
        return str + ")";                          //空表返回()
    }
    ...
    public Node<T> insert(int i, T x) {
        if (x == null) return null;               ////没有插入结点。返回一种执行结果，不是错误，不抛出异常
        Node<T> front = this.head;                 //front指向头结点
        for (int j = 0; front.next != null && j < i; j++)  //寻找第i-1个或最后一个结点（front指向）
            front = front.next;
        front.next = new Node<>(x, front.next); //在front之后插入值为x结点，包括头插入、中间/尾插入
        return front.next;
    }
    ...
    //（4）删除
    public T remove(int i)         //删除第i个元素，0≤i<单链表长度，返回被删除元素。O(n)
    {
        Node<T> front = this.head;                 //front指向头结点
...
//        throw new IndexOutOfBoundsException(i+"");       //抛出序号越界异常
    ...
    //6. 单链表的浅拷贝与深拷贝
    //【思考题2-5】
    //不行    public Unit1.SinglyList(Unit1.SinglyList<? extends T> list)   //深拷贝构造方法，复制单链表list的所有结点
    ...
    public void copy(SinglyList<T> list)         // 复制list所有元素到this，放弃this原结点。O(n)
    {
        ...
        {
            rear.next = new Node<>(p.data, null);//复制结点尾插入到this单链表；对象引用，没有复制
    ...
    public void addAll(SinglyList<T> list) {
        this.concat(new SinglyList<>(list));    //先深拷贝单链表list，再连接复制的list
    }

    //以下返回并集（this+list），即返回分别复制this和list后再连接的单链表，this和list不变
    public SinglyList<T> union(SinglyList<T> list) {
        SinglyList<T> result = new SinglyList<>(this);    //深拷贝this单链表
        result.addAll(list);
