...
{
    public final DoubleNode<T> head;                   //头指针

    public CirDoublyList()                       //构造空循环双链表
    {
        this.head = new DoubleNode<>();         //创建头结点，3个域值均为null
    ...
    public CirDoublyList(T[] values)             //构造循环双链表，由values数组提供元素，尾插入
    {
        ...
        for (T value : values) {
            rear.next = new DoubleNode<>(value, rear, this.head);   //尾插入
            rear = rear.next;
        }
...
//        String str=this.getClass().getName()+"(";   //返回类名
        for (DoubleNode<T> p = this.head.prev; p != this.head; p = p.prev)
            str.append(p.data.toString()).append((p.prev != this.head) ? "," : "");
        return str + ")";                          //空表返回()
    }
    ...
    public void print()                          //输出双向
    {
        ...
        for (DoubleNode<T> p = this.head.next; p != this.head; p = p.next) {
            ...
            if (p.next != this.head) System.out.print(",");
        }
        System.out.print(")，(");
        for (DoubleNode<T> p = this.head.prev; p != this.head; p = p.prev) {
            System.out.print(p.data.toString());
            if (p.prev != this.head) System.out.print(",");
        }
        System.out.println(")");
    }
...
//          System.out.print(p.data.toString()+"？");
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof CirDoublyList<?>)) return false;
        DoubleNode<T> p = this.head.next;
